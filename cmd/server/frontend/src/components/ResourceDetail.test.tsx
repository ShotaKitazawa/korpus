...
describe("ResourceDetail", () => {
  ...
  it("highlights keys in blue", () => {
    const { container } = render(<ResourceDetail resource={mockResource} yaml={sampleYaml} />);
  ...
  it("renders history entries", async () => {
    vi.stubGlobal("fetch", vi.fn().mockImplementation(jsonResponse(historyPage)));
  ...
  it("loads diff when two commits are selected", async () => {
    const mockFetch = vi
      ...
      .mockImplementationOnce(jsonResponse({ before: "kind: Pod\n", after: "kind: Pod\nspec:\n" }));
    vi.stubGlobal("fetch", mockFetch);
  ...
  it("renders diff panes with FROM/TO labels after selecting two commits", async () => {
    const fetchMock = vi
      .fn()
      .mockImplementationOnce(jsonResponse(historyPage))
      .mockImplementationOnce(jsonResponse({ before: "before: yaml\n", after: "after: yaml\n" }));
  ...
  it("resets to yaml tab when resource changes", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockImplementation(jsonResponse({ items: [], total: 0, offset: 0, limit: 50 })),
    );

    const { rerender } = render(<ResourceDetail resource={mockResource} yaml={sampleYaml} />);
